#!/usr/bin/env python3
"""
BoundaryRefiner 训练脚本 | BoundaryRefiner Training Script.
============================================================

极简训练 — 只测一个创新: BoundaryRefiner (粗 Mask → 边界修正 → 精 Mask).
Minimal training — tests ONE innovation: coarse mask → boundary refine → fine mask.

架构 | Architecture:
    FastSAM (frozen) → PureDecoderP3P4 → Coarse Mask
                      → P2 features      ↘
                                    BoundaryRefiner → Fine Mask

消融 | Ablation:
    --no-refine  → PureDecoderP3P4 only (baseline = NoLoRA, 0.7631)
    (default)    → PureDecoderP3P4 + BoundaryRefiner (~85K extra)

用法 | Usage::

    # 主力实验 | Main experiment
    python tools/train/train_boundary_refine.py --epochs 200

    # 消融: 无 Refine | Ablation: no refine (= baseline)
    python tools/train/train_boundary_refine.py --no-refine --epochs 200
"""
...
from adatile.refine import BoundaryRefiner, boundary_aware_loss
...
@torch.no_grad()
def evaluate(decoder, backbone, refiner, dataset, device, use_refine=True, max_samples=0):
    decoder.eval()
    ...
    for idx in tqdm(indices, desc="Eval", leave=False):
        ...
        if use_refine and refiner is not None:
            coarse_4d = coarse.unsqueeze(0)  # [1, C, H/4, W/4]
            refined_4d, _b = refiner(coarse_4d, feats["p2"])
            pred = refined_4d.squeeze(0)  # [C, H/4, W/4]
        else:
            pred = coarse
...
# ═══════════════════════════════════════════════════════════════════
def parse_args():
    p = argparse.ArgumentParser(description="BoundaryRefiner Training")
    ...
    p.add_argument("--no-refine", action="store_true",
                   help="禁用 BoundaryRefiner → PureDecoder 基线")
    p.add_argument("--boundary-weight", type=float, default=2.0,
                   help="边界感知 loss 中边界像素的额外权重 | Extra weight for boundary pixels")
...
def main():
    ...
    if args.output_dir is None:
        ...
        tag = "NoRefine" if not use_refine else f"Refine_BW{args.boundary_weight:.1f}"
        args.output_dir = f"runs/neuseg_{tag}_{ts}"
    out_dir = Path(args.output_dir); out_dir.mkdir(parents=True, exist_ok=True)

    # ── Logger ──
    logger = get_logger("train_boundary_refine")
    logger.add_backend(ConsoleBackend())
    logger.add_backend(FileBackend(str(out_dir / "train.jsonl")))
    logger.log_info("config", f"BoundaryRefiner: refine={use_refine}, bw={args.boundary_weight}")
    ...
    ref_p = sum(p.numel() for p in refiner.parameters()) if refiner else 0
    total_p = dec_p + ref_p
    logger.log_info("model", f"Decoder: {dec_p/1e3:.1f}K | Refiner: {ref_p/1e3:.1f}K | Total: {total_p/1e3:.1f}K ({total_p:,})")

    # ── Optimizer ──
    optim_params = list(decoder.parameters())
    if refiner: optim_params += list(refiner.parameters())
    ...
    logger.log_info("train", f"Refine: {use_refine}, BW: {args.boundary_weight}")
    logger.log_info("train", f"{'='*60}")
    best_miou = 0.0; best_epoch = 0; global_step = 0; nan_count = 0
    rng = random.Random(args.seed)

    for epoch in range(1, args.epochs + 1):
        epoch_losses = []; epoch_ce = []; epoch_dice = []; epoch_ref = []
        ...
        for _ in pbar:
            ...
            decoder.train()
            if refiner: refiner.train()
            feats = backbone(img_dev, extract_proto=True)

            # Coarse mask from decoder
            coarse = decoder(feats["p3"], feats["p4"])  # [C, H/4, W/4]
            ...
            if refiner:
                ...
                refined_4d, boundary = refiner(coarse_4d, feats["p2"])
                pred_prob = refined_4d.squeeze(0)  # [C, H/4, W/4]
            else:
                pred_prob = coarse
            ...
            if boundary is not None:
                # 上采样 boundary 到原图分辨率 | Upsample boundary to original
                boundary_full = F.interpolate(boundary, size=(H, W),
                                              mode="bilinear", align_corners=False).squeeze(1)
                ce = boundary_aware_loss(pred_full.unsqueeze(0), target.unsqueeze(0),
                                         boundary_full.unsqueeze(1), args.boundary_weight)
            else:
        ...
        logger.log_metric("loss", avg_loss, step=epoch, tags=["boundary_refine"])

        if epoch % args.eval_every == 0 or epoch == args.epochs:
            logger.log_info("eval", f"{'─'*50}")
            logger.log_info("eval", f"Evaluation @ Epoch {epoch}")
            result = evaluate(decoder, backbone, refiner, val_ds, device, use_refine=use_refine)
            miou = result["mIoU"]
            ...
            logger.log_metric("mIoU", miou, step=epoch, tags=["boundary_refine"])
            if miou > best_miou:
                ...
                if refiner:
                    ckpt["refiner_state_dict"] = {k: v.clone() for k, v in refiner.state_dict().items()}
    ...
    if refiner:
        final["refiner_state_dict"] = {k: v.clone() for k, v in refiner.state_dict().items()}
    torch.save(final, str(out_dir / "last_model.pt"))

    print(f"\n{'='*60}")
    print(f"  BoundaryRefiner Training — Complete")
    print(f"  Refine: {use_refine}, Trainable: {total_p:,}")
