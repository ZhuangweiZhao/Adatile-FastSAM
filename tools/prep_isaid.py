...
def fix_annotations(json_path: Path, output_path: Path, images_dir: Path,
    ...
    # 实际 JSON categories → 代码 ISAID_CATEGORIES 的 ID 映射
    # Actual JSON category IDs → code ISAID_CATEGORIES ID mapping
    ACTUAL_TO_CODE_ID = {
    ...
    for ann in data.get("annotations", []):
        ...
        if old_id in ACTUAL_TO_CODE_ID:
            ann["category_id"] = ACTUAL_TO_CODE_ID[old_id]
            fixed_cat += 1
