...
class DynamicTileTokenizerImpl(DynamicTileTokenizer):
    ...
    def __init__(
        ...
        self._warmup = False  # warmup mode: force uniform tiling
        ...
        # Global context branch (removed — unused)
        self.global_branch = None
        self._last_plan: Optional[TilePlan] = None

    def set_warmup(self, enabled: bool = True):
        """Enable/disable warmup. Warmup = all cells get tiles (uniform)."""
    ...
    def forward(
        ...
        # Warmup: force uniform tiling — all cells get tiles, decoder learns
        if self._warmup:
            H_s, W_s = H // 32, W // 32
